...
public class TestTablasEnumeradas {
	...
	public static void main(String[] args) throws NamingException, ServiciosException {
		  ...
		  PipelineEstadoUsuarioBean estadoUsuarioQuery = new
		  PipelineEstadoUsuarioBean(); EstadoUsuario estado = new EstadoUsuario();
		  estado.setEstado_valor(EUsuario.DESHABILITADO);
		  estadoUsuarioQuery.getQuery().altaEstado(estado);
		  estado.setEstado_valor(EUsuario.HABILITADO);
		  estadoUsuarioQuery.getQuery().altaEstado(estado); List<EstadoUsuario> eu =
		  estadoUsuarioQuery.getQuery().obtenerPorEstado(EUsuario.HABILITADO);
		  System.out.print(eu.get(0).getId_estado_usuario());
		  
		  PipelineTipoUsuarioBean tipoUsuarioQuery = new PipelineTipoUsuarioBean();
		  TipoUsuario rol = new TipoUsuario(); rol.setRol(TUsuarios.ADMINISTRADOR);
		  tipoUsuarioQuery.getQuery().altaTipoUsuario(rol);
		  rol.setRol(TUsuarios.EXPERTO);
		  tipoUsuarioQuery.getQuery().altaTipoUsuario(rol);
		  rol.setRol(TUsuarios.VOLUNTARIO);
		  tipoUsuarioQuery.getQuery().altaTipoUsuario(rol); List<TipoUsuario> tu =
		  tipoUsuarioQuery.getQuery().obtenerPorRol(TUsuarios.ADMINISTRADOR);
		  System.out.print(tu.get(0).getId_tipo_usuario());
		  
		  
		  PipelineDocumentoCategoriaBean documentoCategoriaQuery = new
		  PipelineDocumentoCategoriaBean(); DocumentoCategoria categoria = new
		  DocumentoCategoria(); for(CDocumento c : CDocumento.values()) {
		  categoria.setCategoria_nombre(c);
