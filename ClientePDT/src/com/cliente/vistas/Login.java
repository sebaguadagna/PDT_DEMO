...
public class Login {
	...
	private void initialize() {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					List<Usuario> userEmail = userEntity.getQuery().obtenerPorUsername(usernameField.getText());
					if(!userEmail.isEmpty()){
						if (userEmail.get(0).getPassword().equals(passwordField.getText())) {
							
							String rol = userEmail.get(0).getRol().getRol().toString();
							System.out.print(rol);
							String email[]  = {userEmail.get(0).getEmail()};
							if(rol.equals(TUsuarios.ADMINISTRADOR.toString())) {
								AdminView.main(null);
								frame.dispose();
							}
							
							if(rol.equals(TUsuarios.EXPERTO.toString())) {
								ExpertoView.main(null);
								frame.dispose();
							}
							
							if(rol.equals(TUsuarios.VOLUNTARIO.toString())) {
								VoluntarioView.main(email);
								frame.dispose();
							}
		...
		lblRegistroDeObservaciones.setBounds(91, 100, 296, 24);
		...
		lblIncendiosInundacionesSequias.setBounds(91, 123, 330, 24);
