...
public class ObservacionBean implements ObservacionBeanRemote {
	...
	public List<Observacion> obtenerPorZona(CZonas zona) throws ServiciosException {
		try {
			TypedQuery<Observacion> query = em.createQuery("SELECT o FROM Observacion o JOIN o.departamento d JOIN d.zona z WHERE z.categoria = :nZona", Observacion.class)
	...
	public List<Observacion> obtenerPorPK(int pk) throws ServiciosException {
		Long pkObs = (long) pk;
		try {
			TypedQuery<Observacion> query = em.createQuery("SELECT o FROM Observacion o WHERE o.pk_observacion = :pk", Observacion.class)
					.setParameter("pk", pkObs);
			return query.getResultList();
			} catch (PersistenceException e) {
				throw new ServiciosException(e.getMessage());
			}
